...
void measure_memory_access_times(volatile int *array, size_t array_size, volatile int *evict_buf, size_t evict_size, uint64_t *min_ns, uint64_t *max_ns, uint64_t timer_freq, volatile uint64_t *latency_buffer, int iteration) {
    ...
    // Maintain a starting index so the warm-up and measurement loops are perfectly symmetrical
    ...
    // 50/50 Balanced Profiling: Alternate between Cache Hits (0) and Main Memory DRAM Hits (1)
...
int main() {
    size_t array_size = 8 * 1024 * 1024; // 32MB large enough to exceed M4 cache size
